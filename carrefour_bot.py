...
def init_db(cursor):
    ...
    cursor.execute("""
    ALTER TABLE productos DROP CONSTRAINT IF EXISTS productos_url_key;
    """)
...
def process_and_save_data():
    ...
    # Deduplicar productos por ID en memoria para evitar errores de actualización múltiple en una misma transacción
    productos_unicos = {}
    for p in all_products_today:
        productos_unicos[p['id']] = p

    productos_data = [(p['id'], p['nombre'], p['categoria'], p['url']) for p in productos_unicos.values()]
